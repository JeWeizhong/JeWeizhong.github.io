...
- [ ] 如果你有输了个命令行，但是你改变注意了，但你又不想删除它，因为你要在历史命令中找到它，但你也不想执行它。那么，你可以按下 `Alt_#` ，于是这个命令关就被加了一个#字符，于是就被注释掉了。
...
- [ ] Apache的一个叫 `ab` 的工具是一个很有用的，用`quick-and-dirty`的方式来测试网站服务器的性能负载的工作。如果你需要更为复杂的测试，你可以试试 `siege`。

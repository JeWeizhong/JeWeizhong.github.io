...
工作原理是：有一组已经分类好的数据。输入没有标签的新数据后，将新数据的每个特征与样本集中数据对应的特征进行比较，然后算法提取样本集中特征最相似数据（最近邻）的分类标签。
...
if __name__ == '__main__':
    ...
    print(result) # B
```
计算$(x_1,y_1)$与$(x_2,y_2)$距离公式：
$$  \sqrt{(x_1 - x_2)^2+(y_1-y_2)^2}    $$
这个距离也叫欧式距离。
上述代码的`classify0`函数中17~21行就是计算距离的方法

## 示例一：改进约会网站的配对效果

*为了保证代码完整性，保留了`classify0`,`createDataSet`,是下列代码能够作为一个脚本单独运行，同时将多余的注释删除，节省空间*
...
下面我们来进行代码解析，`showdatas`函数是从网上看到的实质上就是将数据可视化，看不懂可以暂时先不管。
首先是`file2matrix`函数读取数据转换成矩阵格式，最后一列就是标签
```
# print(datingDataMat)
...
```
可视化后应该是计算两点间的距离，但是，三个数值之间的差异很大，也就是说飞行的里程数占的比重比其他的要大，但是我们在这里认为这三个因素都应该是平等的，因此就要对数据进行归一化处理，所以就有了`autoNorm`函数。

此函数用到的算法是
$$new = (old-min)/(max-min)$$

这样就将里程数这个特征值控制在0~1之内，以保证三个特征值的取值范围是一样的，即都有一样的权重。(*好像对特征值进行缩放会有损失，暂时先不管*)

最后我们就要去验证这个算法的可靠性了，也就是最后一个函数`classifyPerson`

这个示例是用了90%的样本来训练，然后用剩下的10%去做验证，后面的算法中也有更高级的数据分组。这也就是`datingClassTest`函数的功能了

---

##示例二：手写识别系统

为了使用前面两个例子的分类器，书中将一个32×32的二进制图像变成了文本存储，这里只附上代码，由于方法都一样，就不在赘述
```
import numpy as np
import operator
from os import listdir
...
[`sklearn.neighbors`](http://scikit-learn.org/stable/modules/classes.html#module-sklearn.neighbors)模块就是k-近邻算法，这里只介绍一个函数[`KNeighborsClassifier`](http://scikit-learn.org/stable/modules/generated/sklearn.neighbors.KNeighborsClassifier.html#sklearn.neighbors.KNeighborsClassifier)
...
---
**参考链接：**http://blog.csdn.net/c406495762
